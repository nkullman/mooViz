...
/**
 * Handles clicks on drawing elements
 */
function clickToggleSelected(mvid) {
    // get graph objects corresponding to this solution
    var ades = d3.selectAll(".actionableDrawingElement."+mvid);
    if (selectedSolutions.indexOf(mvid) > -1) {
        // already in solutions, so we remove it
        selectedSolutions.splice(selectedSolutions.indexOf(mvid), 1);
        // unclass the graph objects
        ades.classed("selected", false);
    } else {
        // not in solutions, so add it
        selectedSolutions.push(mvid);
        // class graph objects
        ades.classed("selected", true);
    }
}